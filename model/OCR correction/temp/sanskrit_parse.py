...
root = get("http://sanskritdocuments.org/", headers={'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.75 Safari/537.36'})
...
temp_html_link = []
for link1 in lns:
    pg_new = get(link1, headers={'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.75 Safari/537.36'})
    ...
    for link2 in l2:
        if(link2.startswith("./")):
            if ".html" in link2:
                temp_html_link.append("http://sanskritdocuwments.org/" + link2[2:])
html_link = []
for link in temp_html_link:
	html_link.append("https"+link[4:])
